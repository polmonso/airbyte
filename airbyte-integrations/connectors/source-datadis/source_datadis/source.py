...
class DatadisStream(HttpStream, ABC):
    ...
    `class DatadisStream(HttpStream, ABC)` which is the current class
    `class Customers(DatadisStream)` contains behavior to pull data for customers using v1/customers
    `class Employees(DatadisStream)` contains behavior to pull data for employees using v1/employees

    If some streams implement incremental sync, it is typical to create another class
    `class IncrementalDatadisStream((DatadisStream), ABC)` then have concrete stream implementations extend it. An example
    is provided below.
...
class Employees(IncrementalDatadisStream):
    """
...
# Source
class SourceDatadis(AbstractSource):
    ...
    def streams(self, config: Mapping[str, Any]) -> List[Stream]:
        ...
        """
        # TODO how to deal with non-auth, short-lived tokens?
        # token = asyncio.run(get_token(config['username'], config['password']))
        # auth = TokenAuthenticator(token=token)  # Oauth2Authenticator is also available if you need oauth support
        auth = NoAuth()
        return [GetConsumptionData(authenticator=auth, config=config)]
